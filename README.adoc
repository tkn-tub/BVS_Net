= Forward Error Correction Codes in Intra-body Links through ns-3 flow : Development of FEC Module in ns-3 and Integrating into BVS Simulator
Yerim Lee <yerim.lee@campus.tu-berlin.de>
2024-02-14

Forward Error Correction Codes for a better networking in BVS Simulator
...
As shown in this project, BVS with Forward Error Correction (FEC) Codes serves as a method for detecting and correction errors in the communicstion link. In 2 seperate classes, which are nanobots and gateway respectively, there are 2 seperate FEC encoder and decoder, that respectively encodes transmitted data packets and detects/corrects errors during transmission. These FEC attributes are appropriately determined according to the Wireless Sensor Network (WSN) channel characteristics such as Bit Error Rate (BER) and the degree of error burstiness. Therefore, FEC modules not only detects and corrects errors in the communication link module developed thus far but is also expected to significantly enhance the overall quality of future nanoscale communication modules.


== New features
 
  - FEC Module using Hamming (12, 8) encoder and decoder
  - Outputs the received packages into a csv file (gateway_data.csv)
  - Calculates the bit error rate per transmission (gateway_ber_global.csv)
...
== New Parameters
  ...
  - initial transmit power: POWER 10^2
...
The simulation returns:
	...
	- a csv-file (/ns-3.40/gateway_data.csv) with the transmitted data of the simulated nanobots.
	- a csv-file (/ns-3.40/gateway_ber_glob.csv) with the calcualted ber.
